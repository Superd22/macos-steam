...
**Route A has since been built and measured on the real title — see §8**, which is where this
document's one large [REASONED] claim (§2.1, "this fits the DRM route with almost no new code")
became measured, and where the implementation's one deliberate departure from §2 is recorded.
Everything before §8 is the research as it stood before any of it was written, deliberately: the
argument that justified the code should stay readable next to the code.
...
**Should become issues:** all three were filed, and the state of each is recorded here rather than
left to be inferred — §8 is the first one's answer.
...
- **Build Route A into the shim** (the §2.1 change) and measure it end-to-end on Space Marine 2
  ...
  the injector's, not the overlay's. — **#112, done; see §8.**
- **Fold the corrected bail condition into ADR 0003 / `overlay-injection.md`.** The deadline is a
  ...
  no longer a dead wrong turn once the installer is called. — **#113, done: ADR 0003's
  2026-09-05 addendum and `overlay-injection.md` §3.**
- **Route B as a general (non-DRM) fallback** for any title where import[0] is contested (e.g.
  the same anti-tamper titles the injector loses), tracked separately. — **#114, open.**

---

## 8. Addendum, 2026-09-05: Route A shipped, and what changed on the way (#112)

Route A is built (`overlay_arm_late()` in `src/shim/shim_unix.cpp`, plus the interlock change in
`steamclient-shim-launch.sh`) and measured end-to-end on Space Marine II — **the one run §7 named
as the follow-up and this research deliberately did not do.** The §2 mechanism survived contact
with the real title unchanged. One thing was added that is not in §2, and it is load-bearing.

### 8.1 The addition: arm only when `NSApp` is already non-nil [MEASURED]

§2's harness calls the installer unconditionally, which is correct for a harness and wrong for
shipped code: the same constructor runs on the **injector** path, where the renderer arrives
*before* `NSApplication` and Valve's own swizzle is going to fire on its own. Calling the
once-guarded installer there would burn the guard early, at a moment nothing in §1 says is safe,
and put the entire non-wrapped library's overlay — the thing that already works — behind an
untested ordering.

So the shipped version reads `NSApp` (`dlsym(RTLD_DEFAULT, "NSApp")`, dereferenced) and arms only
when it is non-nil. That is exactly the predicate §1.1 implies: `NSApp` is set from inside
`-[NSApplication init]`, so **non-nil means the trigger has already fired and can never fire
again** — the hooks are ours to install or nobody's — and **nil means Valve's swizzle still will
fire**, so the right move is to keep our hands off. A process with no AppKit at all reads nil too,
which is also right: nothing there to hook.

It is asked of AppKit rather than of our own route on purpose. "Am I on the DRM route" is a
question about our configuration; "has `init` already run" is the question the mechanism actually
turns on, and it stays correct for arrangements nobody has thought of yet. Reading the exported
symbol is the only way to ask it without *answering* it — `[NSApplication sharedApplication]`
would construct the very instance being tested for.

Both branches are observed, on the same build:

```
DRM route, SM2          overlay: NSApp already up — calling -[NSApplication init] IMP
                        0x215b705d0 (in .../gameoverlayrenderer.dylib) with self=nil
injector route, probe   overlay: NSApp not up yet — leaving the arming to Valve's own
                        -[NSApplication init] swizzle (the early/injector path)
```

Two smaller departures, neither behavioural: the IMP is located by asking `dladdr` which of the two
selectors lands inside the renderer image (§2 does this too, but the shipped code states *why* —
which selector carries Valve's code depends on whether the swizzle has already exchanged them), and
every failure is a log line rather than an abort, because the overlay must never be why a title
does not launch.

### 8.2 The end-to-end run, on the real title [MEASURED]

Space Marine II (2183900), 2026-09-05, launched the way this machine launches it — the wrapped
`…\client_pc\root\bin\pc\Warhammer 40000 Space Marine 2 - Retail.exe` directly through
`wineloader`, bypassing `start_protected_game.exe` and EasyAntiCheat — on a **rotated**
`shim-unix.log` (that log is append-only and does not stamp pids on its Steamworks lines, which is
how an earlier investigation fooled itself):

| run | `SHIM_OVERLAY` | `Hooking` | past the DRM wrapper |
|---|---|---|---|
| pid 2904 | 1 | **5**, `winemac` present | yes |
| pid 5220 (negative control) | 0 | **no renderer log written at all** | yes |

"Past the wrapper" is title-level Steamworks traffic, not `CreateInterface`: the wrapper itself
calls `SteamClient017` and `SteamClient014` before the title gets control, and pid 2904's log
carries the title's own `SteamClient020`, `GetAppID() -> 2183900`, `GetCurrentGameLanguage() ->
english`, `SteamInput Init(1)`, `ISteamFriends::SetRichPresence`, `ISteamUser::GetAuthTicketForWebApi`
and the callback pump. The renderer log for that pid carries the five `Hooking` lines and then
`ValveGetScreenSize( 1512, 982 )` / `ValveGetOutputBounds` / `SetScaleFactors` — the renderer
tracking the title's drawable, frame after frame.

The A/B that isolates the call itself was run on `d3dprobe` with the new `SHIM_LATE_PULL=1`
(see `instruments/overlay-probe/README.md`), which puts the probe in the wrapped title's timing
without needing a title:

| arrangement | `winemac` at attach | `Hooking` |
|---|---|---|
| late pull, this unixlib | present | **5** |
| late pull, the unixlib without §8.1's call | present | **0** |
| late pull, `SHIM_OVERLAY=0` | — | no renderer log at all |
| early self-pull (ADR 0003's ordering), this unixlib | absent | **5** |
| through `overlayinject64.exe`, `SHIM_NO_SELF_PULL=1` | absent | **5** |

Rows 1 and 2 differ only by the arming call. Rows 4 and 5 are the non-regression that mattered
most: the gate declines and the ordinary path is what it was.

### 8.3 What is still not measured

The three items §7 listed as reasoned are **not** all discharged, and shipping Route A did not
discharge them:

- **Anti-tamper tolerance against EasyAntiCheat: still [REASONED].** The SM2 run above goes around
  `start_protected_game.exe`, as every SM2 measurement in this repo does. The argument — no import
  rewrite, no suspended process, no remote thread, in-process at runtime, the title's own bytes
  untouched — is unchanged and unverified.
- **Input parity on the late path: still unmeasured.** `instruments/overlay-probe/input-parity-run.sh`
  needs an operator at the keyboard. §1.1's claim that the input swizzles are installed from the
  renderer's own load path rather than from the `init` swizzle remains [REASONED] from the
  disassembly.
- **Nobody has seen the panel on a wrapped title.** What §8.2 shows is that the five hooks are
  installed and the renderer is tracking the drawable every frame. The hotkey needs a human, so
  "hooked" is proven and "armed" (in `CONTEXT.md`'s sense) is not, for this route.

The `re-verify-on` at the top of this document now guards shipped behaviour rather than a finding:
a Steam client that drops the `-[NSApplication init]` swizzle takes the wrapped population's
overlay with it. The shipped code fails safe when that happens — it logs that no IMP was found
inside the renderer and the title launches without an overlay — and §3's Route B is still the
answer if it does.
