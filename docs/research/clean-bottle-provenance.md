...
1. **The minimum load-bearing state is exactly one registry value:**
   ...
   called. (They may matter *later*, for connect-to-user or DRM; that's #12's territory.)
2. **⚠️ The "local steamclient64.dll next to the EXE" path never fires.** #4 read the
   ...
   itself ever behaves differently, re-test. The registry hook works regardless.)
