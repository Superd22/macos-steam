...
Unmapped controls staying at **14** with the gate ON prove this is a genuine per-app effect
...
`nonexistent-probe-INVALID-oracle` verbatim — a string that exists only in the planted file
— an unambiguous parse oracle.

## Priority model (load-bearing for the per-title override question)
