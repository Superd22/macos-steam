...
if stale "$REPO/tools/compat-enabler/libcompat-enabler.dylib" \
         "$REPO/tools/compat-enabler/enabler.c" \
         "$REPO/tools/compat-enabler/build.sh"; then
    log "building injector"; "$REPO/tools/compat-enabler/build.sh"
fi
if [ ! -f "$REPO/tools/shim/dist/steamclient.dll" ] \
   || stale "$REPO/tools/shim/dist/steamclient64.dll" \
            "$REPO/tools/shim/shim_pe.c"      "$REPO/tools/shim/shim_unix.cpp" \
            "$REPO/tools/shim/shim_abi.h"     "$REPO/tools/shim/shim_vtables.h" \
            "$REPO/tools/shim/steam_ifaces.h" "$REPO/tools/shim/build.sh"; then
    log "building shim (both bitnesses)"; "$REPO/tools/shim/build.sh"
fi
if [ ! -f "$REPO/tools/overlay-inject/dist/overlayinject32.exe" ] \
   || stale "$REPO/tools/overlay-inject/dist/overlayinject64.exe" \
            "$REPO/tools/overlay-inject/overlayinject.c" \
            "$REPO/tools/overlay-inject/build.sh"; then
...
{
printf '%s\n' '#!/bin/sh'
printf '%s\n' '# Generated by tools/installer/install.sh — do not edit; reinstall instead.'
