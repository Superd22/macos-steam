...
| `shimprobe/` | does a decoy `steamclient64.dll` actually get loaded in a clean bottle? Answer in `docs/research/clean-bottle-provenance.md`; `FINDINGS.md` here is a pointer to it. |
| `overlay-probe/` | is Valve's macOS overlay reachable from a process we control, and what gates it? **ADR 0003**; detail in `docs/research/overlay-injection.md`. |
